...
def video_data(good, bad, offset):
    good_vid_kp, frames = read_keypoints_from_csv(f'keypoints {good}.csv')
    bad_vid_kp, frames = read_keypoints_from_csv(f'keypoints {bad}.csv')
    all_kp = []
    if offset < 0:
        offset = abs(offset)
...
def model_good_swing(good_video, bad_video, random_videos, epochs_run):
    # - ==> 2nd vid ahead
    # + ==> 1st vid ahead
    offset = minimize_difference(f'keypoints {good_video}.csv', f'keypoints {bad_video}.csv')
    ...
    for i in random_good_videos:
        kp, frames = get_keypoints_from_csv(f'keypoints {i}.csv', offset)
    ...
    good_test, frames = get_keypoints_from_csv(f'keypoints {good_video}.csv', offset)
    bad_test, frames = get_keypoints_from_csv(f'keypoints {bad_video}.csv', offset)
    ...
    for j in range(num_frames):
        ...
        if j != 0:
            ...
            bad_kp, frames = read_keypoints_from_csv(f'keypoints {bad_video}.csv')
            for i in range(17):
                x = bad_kp[i][j][0]
                y = bad_kp[i][j][1]
                keypoint_predictions.append((x, y))
